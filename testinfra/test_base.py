...
def test_logrotate(host):
    ...
    if ansible_vars['inventory_hostname'] == 'bridge.openstack.org':
        ...
        assert cfg_file.exists
        assert b'/var/log/ansible/ansible.log {' in cfg_file.content
        cfg_file = host.file("/etc/logrotate.d/run_all_cron.log.de808.conf")
        assert cfg_file.exists
        assert b'/var/log/ansible/run_all_cron.log {' in cfg_file.content
