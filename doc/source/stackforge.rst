...
configuring Gerrit/Jenkins/Zuul etc. The opencontrail-infra team can
...
Edit ``modules/opencontrail_project/files/review.projects.yaml`` and add a
...
``modules/opencontrail_project/files/gerrit/acls/stackforge/project-name.config``
...
In the same opencontrail-infra/config repository (and in the same change
...
``modules/opencontrail_project/files/jenkins_job_builder/config/projects.yaml``
...
should look something like::
  ...
  - project:
      ...
      node: bare-precise
      tarball-site: tarballs.opencontrail.org

      jobs:
        - python-jobs

List of jobs included to the ``python-jobs`` jobs group is located in
``modules/opencontrail_project/files/jenkins_job_builder/config/python-jobs.yaml``.
...
appropriate. Edit
``modules/opencontrail_project/files/zuul/layout.yaml``
...
That concludes the bare minimum opencontrail-infra/config changes necessary to
add a project to StackForge. You can commit these changes and submit
them to review.opencontrail.org at this point, or you can wait a little
...
https://bugs.launchpad.net/opencontrail-ci/+filebug (make sure to mention
the Gerrit full name or E-mail address of your initial member). See
https://wiki.opencontrail.org/wiki/Project_Group_Management for details on
...
change to review.opencontrail.org.
...
review and approve.::
  ...
  git clone https://git.opencontrail.org/stackforge/project-name
  cd project-name
