...
1. Log Pusher Scripts.
   Subscribes to the Zuul Ansible Launcher ZeroMQ Event Publisher listening
   for build finished events. When a build finishes an event is received from
   Zuul which is then converted into Gearman jobs specific to that
   ...
   up, annotate them with Zuul build info and finally sends them to a
...
This is a pair of simple Python scripts. The first listens to Zuul
...
* Zuul publishes build complete notifications.
* Receive notification from Zuul and convert to Gearman jobs.
...
:cgit_file:`modules/openstack_project/files/logstash/log-client.yaml`
