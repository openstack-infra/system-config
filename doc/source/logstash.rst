...
cluster on a large VMs to give ElasticSearch both memory and disk space.
Per index (Logstash creates one index per day) we have multiple replicas
to distribute disk utilization and provide high availability. Each
replica is broken into five shards (each shard is basically its own
index), so as to increase indexing throughput.
