...
jobs are not being created in jenkins masters, the play is to run jjb manually on the
masters that fail. To do this::

  user@jekins01# sudo -H jenkins-jobs --conf /etc/jenkins_jobs/jenkins_jobs.ini \
...
of all jobs, the cache needs to be ignored. To do this::
  ...
  user@jekins01# sudo -H jenkins-jobs --ignore-cache --conf \
...
`https://jenkins0[1-7].openstack.org/quietDown`
