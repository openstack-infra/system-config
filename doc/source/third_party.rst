...
`zuul sample <https://git.openstack.org/cgit/openstack-infra/zuul/tree/etc/layout.yaml-sample>`_
...
following the `layout documentation <http://ci.openstack.org/zuul/zuul.html#layout-yaml>`_

After a gerrit event matches a pipeline, zuul will look at the project identified
in that gerrit event and invoke the jenkins jobs specified in the projects section (for the
matching pipeline) using the `Jenkins Gearman Plugin
...
`The layout.yaml used by Openstack is a good reference for pipeline definitions,
...
constructs these XML configuration files after reading a
set of YAML files and job templating rules.
...
<https://github.com/openstack-infra/project-config/blob/master/jenkins/jobs>`_
(The project.yaml file is a good starting point)
