...
Here’s a snippet from that file that constructs the ``check`` pipeline taken from
...
`Full details of each configuration option is available
<http://ci.openstack.org/zuul/zuul.html#layout-yaml>`_
...
pipeline) using the `Jenkins Gearman Plugin
...
`The layout.yaml used by Openstack
...
`Basic overview of using JJB <http://ci.openstack.org/jjb.html#yaml-format>`_

`Official Documentation to defining Jenkins jobs using JJB
<http://ci.openstack.org/jenkins-job-builder/definition.html>`_

`The JJB description of all jobs used by Openstack
<https://github.com/openstack-infra/project-config/blob/master/jenkins/jobs/projects.yaml>`_
