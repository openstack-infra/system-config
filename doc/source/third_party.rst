...
* Support recheck to request re-running a test.
  ...
  * Commands specific to a 3rd party system **must** be namespace
    prefixed with the system in question. ``3rdparty-name:
    command``. Specifically 3rd party systems **must not** extend
    existing recheck/check syntax with 3rd party specific commands.
