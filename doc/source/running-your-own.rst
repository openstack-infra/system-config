...
* Optionally a service account for your Jenkins nodes (separation of concerns -
  this account has it's credentials loaded into the cloud itself). You can run
...
Then start up your puppet dashboard::
...
* ssh into the new node and update it's ``/etc/default/puppet`` to autostart
  per :file:`launch/README`
