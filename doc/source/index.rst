.. OpenContrail Infrastructure documentation master file, created by
   sphinx-quickstart on Mon Jul 18 13:42:23 2011.
   You can adapt this file completely to your liking, but it should at least
   contain the root `toctree` directive.

OpenContrail Project Infrastructure
================================

This documentation covers the installation and maintenance of the
project infrastructure used by OpenContrail.  It may be of interest to
people who may want to help develop this infrastructure or integrate
their tools into it.  Some instructions may be useful to other
projects that want to set up similar infrastructure systems for their
developers.

OpenContrail developers or users do not need to read this documentation.
Instead, see http://wiki.opencontrail.org/ to learn how contribute to or
use OpenContrail.

Contents:

.. sidebar:: HOWTOs

   * :doc:`third_party`
   * :doc:`stackforge`
   * :doc:`running-your-own`

.. toctree::
   :maxdepth: 2

   project
   test-infra-requirements
   sysadmin
   systems

.. toctree::
   :hidden:

   running-your-own
   stackforge
   third_party


Indices and tables
==================

* :ref:`genindex`
* :ref:`modindex`
* :ref:`search`

