...
class SubunitRetriever(threading.Thread):
    ...
    def _handle_event(self):
        ...
        try:
            ...
            if fields['build_status'] != 'ABORTED':
                ...
                # discarded by zuul.
                subunit_io = self._retrieve_subunit(source_url, retry)
    ...
    def _subunit_1_to_2(self, raw_file):
        ...
        # NOTE(mtreinish): This fun nested call is how you convert a
        # subunit v1 stream to a v2 stream...
        subunit.filters.run_tests_from_stream(
            subunit.filters.find_stream(raw_file, []),
            testtools.ExtendedToStreamDecorator(
                subunit.StreamResultToBytes(buf)))
        return buf
...
class Subunit2SQLProcessor(object):
    ...
    def handle_subunit_event(self):
        ...
        subunit2sql.shell.CONF([], project='subunit2sql',
                               default_config_files=[self.config])
        subunit2sql.shell.CONF.set_defaults('run_meta', subunit)
        logging.debug('Converting Subunit V2 stream to SQL')
        stream = subunit2sql.subunit.ReadSubunit(subunit_v2)
        subunit2sql.shell.process_results(stream.get_results())
...
class Server(object):
    ...
    def setup_processor(self):
        ...
        subunit2sql_config = self.config['subunit2sql_config']
