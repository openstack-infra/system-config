...
class Subunit2SQLProcessor(object):
    ...
    def handle_subunit_event(self):
        ...
        shell.cli_opts()
        shell.parse_args([])
        # Set connection info and state_dir from config
        db_connection = self.config['connection']
        state_path = self.config('state_path')
        shell.CONF.set_default('connection', db_connection, group='database')
        shell.CONF.set_default('state_path', state_path)
        # Set run metadata from gearman
        shell.CONF.set_default('run_meta', subunit)
