...
# creation date or the version number
...
# date or comments.
# Note that this grep checks for comments as well
if [ `git diff --cached | egrep -v "(POT-Creation-Date|Project-Id-Version|^[\+\-]#|^\+{3}|^\-{3})" | egrep -c "^[\-\+]"` -gt 0 ]
then
    # Commit and review
    git commit -F- <<EOF
$COMMIT_MSG
