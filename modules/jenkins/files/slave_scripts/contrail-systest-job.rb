...
def setup_sanity
    vm = @vms.first
    Sh.run "ssh #{vm.vmname} \"(source /opt/contrail/api-venv/bin/activate && source /etc/contrail_profile && pip install fixtures testtools testresources selenium pyvirtualdisplay)\""
...
def run_sanity
    Sh.run "ssh #{@vms.first.vmname} \"(source /etc/contrail_profile && /usr/local/jenkins/slave_scripts/ci-infra/contrail_fab run_sanity:quick_sanity)\""
end
