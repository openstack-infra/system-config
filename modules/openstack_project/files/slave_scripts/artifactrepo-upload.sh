...
function show_options () {
    ...
    echo "-u URL (url of the artifacts repository. For jenkins and java packages it defaults to upstream repositories, for pypi it defaults to tarballs.openstack.org)"
    echo "-c CREDENTIALS_FILE (credentials file, contains user/password to access repository)"
    echo "-p PROJECT (project name)"
    echo "-m METADATA_FILENAME (metadata file, this is only used for jenkins and java package uploads)"
    echo "-f ARTIFACT_FILENAME (artifact file to upload, this is only used for jenkins and java package uploads)"
}
...
function set_parameters () {
    ...
    java-package)
        ...
        ;;
    pypi-tarball)
        URL=${URL:-"http://tarballs.openstack.org"}
...
function upload_artifact () {
    ...
    pypi-tarball)
        rm -rf *tar.gz
        curl --fail -o $ARTIFACT_FILENAME "${URL}/${PROJECT}/${ARTIFACT_FILENAME}"
