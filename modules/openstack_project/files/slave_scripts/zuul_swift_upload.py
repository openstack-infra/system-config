...
def swift_form_post_submit(file_list, url, hmac_body, signature):
    ...
    for sub_file_list in (file_list[pos:pos + payload['max_file_count']]
        for pos in xrange(0, len(file_list), payload['max_file_count'])):
        files = {}
...
if __name__ == '__main__':
    ...
    for file_path in args.files:
        ...
        file_list += build_file_list(file_path, logserver_prefix, log_path)

    create_root_index = True
    index_file = ''
    if create_root_index:
        index_file = make_index_file(, logserver_prefix,
                                     swift_destination_prefix)
        file_list.append({
            'filename': os.path.join(os.path.dirname(relative_name),
                                     os.path.basename(index_file)),
    ...
    swift_form_post_submit(file_list, swift_url, swift_hmac_body, swift_signature)
