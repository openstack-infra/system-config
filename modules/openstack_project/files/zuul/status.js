...
function toggle_display_jobs(_header) {
    ...
    content.slideToggle(100, function () {
        ...
        collapsed_index = window.zuul_collapsed_changes.indexOf(changeid);
        if (content.is(":visible")) {
            window.zuul_expanded_changes.push(changeid);
            if (collapsed_index > -1) {
                window.zuul_collapsed_changes.splice(collapsed_index, 1);
            }
        } else {
            window.zuul_collapsed_changes.push(changeid);
            if (expanded_index > -1) {
                window.zuul_expanded_changes.splice(expanded_index, 1);
            }
        }
    });
}
...
function toggle_expand_by_default(_checkbox) {
    ...
    $.each($('div.change'), function(i, _change) {
        ...
        expanded_index = window.zuul_expanded_changes.indexOf(change.attr('id'));
        collapsed_index = window.zuul_collapsed_changes.indexOf(change.attr('id'));
