...
describe 'openstack_project::server' do
  ...
  def preconditions_puppet_manifest
    manifest_path = File.join(pp_path, 'preconditions.pp')
    File.read(manifest_path)
  end
  ...
  it 'should turn root ssh back on' do
    ...
    shell('sed -i -e "s;HostKey /etc/ssh/ssh_host_dsa_key;HostKey /etc/ssh/ssh_host_dsa_key\nHostKey /etc/ssh/ssh_host_ecdsa_key;" /etc/ssh/sshd_config')
    shell('sed -i -e "s;HostKey /etc/ssh/ssh_host_ecdsa_key;HostKey /etc/ssh/ssh_host_ecdsa_key\nHostKey /etc/ssh/ssh_host_ed25519_key;" /etc/ssh/sshd_config')
