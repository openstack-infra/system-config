...
class openstack_project::static (
  ...
  file { '/usr/local/sbin/log_compression_and_deletion.sh':
    ensure => present,
    owner  => 'root',
    group  => 'root',
    mode   => '0744',
    source => 'puppet:///modules/openstack_project/log_compression_and_deletion.sh',
  }
  ...
  cron { 'gziprmlogs':
    ...
    minute      => '0',
    hour        => '*/6',
    command     => 'bash /usr/local/sbin/log_compression_and_deletion.sh',
    environment => 'PATH=/usr/bin:/bin:/usr/sbin:/sbin',
    require     => File['/usr/local/sbin/log_compression_and_deletion.sh'],
  }
