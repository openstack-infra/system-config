...
class openstack_project::subunit_db_server (
  ...
  exec { 'create_db_user':
    command => "/usr/local/bin/subunit_db__user.sh $subunit2sql_db_user $subunit2sql_db_pass $subunit2sql_db_name",
    require => [
        Db[$subunit2sql_db_name],
        File['/usr/local/bin/subunit_db__user.sh'],
    ],
  }
