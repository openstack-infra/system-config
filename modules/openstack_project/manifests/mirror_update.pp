...
class openstack_project::mirror_update (
  ...
  class { '::openstack_project::reprepro':
    ...
    distributions => 'openstack_project/reprepro/distributions.erb',
    releases => ['trusty'],
  }
  ...
  cron { 'reprepro ubuntu':
    user        => $user,
    hour        => '*/2',
    command     => 'flock -n /var/run/reprepro/mirror.lock reprepro-mirror-update /etc/reprepro/ubuntu mirror.ubuntu >>/var/log/reprepro/mirror.log 2>&1',
    environment => 'PATH=/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin',
    require     => [
       File['/usr/local/bin/reprepro-mirror-update'],
       File['/etc/afsadmin.keytab'],
