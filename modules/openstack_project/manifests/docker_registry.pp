...
class openstack_project::docker_registry(
  ...
  file { '/usr/local/bin/docker-registry-wrapper':
    ensure  => present,
    source  => 'puppet:///modules/openstack_project/docker_registry/wrapper.sh',
    mode    => '0755',
    require => Package['docker-registry']
  }

  file { '/etc/default/docker-registry':
    ensure  => present,
    source  => 'puppet:///modules/openstack_project/docker_registry/default',
    require => Package['docker-registry']
  }
