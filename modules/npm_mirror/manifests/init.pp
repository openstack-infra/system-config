...
class npm_mirror(
  ...
  exec { 'ensure_database_exists':
    unless  => "curl --netrc-file /etc/couchdb/.netrc --output /dev/null --silent --head --fail http://${host}:${port}/registry",
    command => "curl --netrc-file /etc/couchdb/.netrc --silent -X PUT http://${host}:${port}/registry",
    path    => '/usr/local/bin:/usr/bin:/bin',
    require => [
      Exec['wait_for_couchdb'],
      File['netrc']
  ...
  exec { 'start_replication':
    unless  => "curl --netrc-file /etc/couchdb/.netrc --output /dev/null --silent --head --fail http://${host}:${port}/_replicator/replicate_npm",
    command => "curl --netrc-file /etc/couchdb/.netrc --silent -d @/etc/couchdb/replicate_npm.json -X POST http://${host}:${port}/_replicator -H 'Content-Type: application/json'",
    path    => '/usr/local/bin:/usr/bin:/bin',
    require => [
      Exec['ensure_database_exists'],
      File['netrc'],
