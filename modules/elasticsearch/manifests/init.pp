...
class elasticsearch (
  ...
  exec { 'get_elasticsearch_deb':
    command => "wget https://download.elasticsearch.org/elasticsearch/elasticsearch/elasticsearch-${version}.deb -O /tmp/elasticsearch-${version}.deb && sha1sum /tmp/elasticsearch-${version}.deb > /tmp/elasticsearch-${version}.deb.sha1.gen",
    path    => '/bin:/usr/bin',
