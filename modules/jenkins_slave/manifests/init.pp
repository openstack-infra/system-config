class jenkins_slave {
    ...
    cron { "updatepuppet":
      user => root,
      minute => "*/15",
      command => "cd /root/openstack-ci-puppet && /usr/bin/git pull >/dev/null 2>&1 && /var/lib/gems/1.8/bin/puppet apply -l /tmp/manifest.log --modulepath=/root/openstack-ci-puppet/modules manifests/site.pp",
    ...
    cron { "updateci":
      user => jenkins,
      minute => "*/15",
      command => "cd /home/jenkins/openstack-ci && /usr/bin/git pull origin master >/dev/null 2>&1"
    }
