define planet::site($git_url) {
  ...
  file { "/etc/nginx/sites-available/paste-${name}":
    ensure => present,
  ...
  file { "/etc/nginx/sites-enabled/paste-${name}":
    ensure => link,
    target => "/etc/nginx/sites-available/paste-${name}",
    require => Package[nginx],
  }

# if we already have the mercurial repo the pull updates

  exec { "update_${name}_paste":
    command => "git pull",
