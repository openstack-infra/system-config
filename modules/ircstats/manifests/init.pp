...
class ircstats {
  ...
  # superseriousstats install directory
  file { '/opt/ircstats':
    ensure  => directory,
    owner   => 'ircstats',
    group   => 'ircstats',
  ...
  vcsrepo { '/opt/ircstats/superseriousstats':
    ensure   => latest,
    provider => git,
    revision => 'master',
    source   => 'https://github.com/tommyrot/superseriousstats.git',
  }
  ...
  cron { 'ircstats':
    ...
    hour    => '*',
    minute  => '0',
    command => '/usr/bin/python /usr/local/bin/ircstats.py -l /var/lib/statusbot/www -t /etc/ircstats/ircstats.conf.template -i /var/lib/statusbot/www/stats',
    require => Vcsrepo['/opt/ircstats/superseriousstats'],
  }
}
