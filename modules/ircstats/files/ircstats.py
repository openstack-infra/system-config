...
SSS_DOWNLOAD_URL = 'http://sss.dutnie.nl/sss-7.0.tgz'
...
class SuperSeriousStats:
    ...
    def install(self):
        ...
        package_path = os.path.join(self.install_base,
                                    '%s' % SSS_DOWNLOAD_URL.split('/')[-1])
        exec_command('tar zxf %s --strip-components 1' % package_path,
                     cwd=self.__create_dir(self.install_dir))
    ...
    def install_supporting_files(self):
        ...
        for file in files:
            source = os.path.join(self.install_dir, 'www', file)
...
def main():
    ...
    package_path = os.path.join(opts.install_base,
                                '%s' % SSS_DOWNLOAD_URL.split('/')[-1])
    if not os.path.exists(package_path):
        download_from_url(SSS_DOWNLOAD_URL, package_path)

    if not opts.channel:
        channels = [d for d in os.listdir(opts.irclog_base)
                    if os.path.isdir(os.path.join(opts.irclog_base, d))]
    else:
