...
class etherpad_lite::mysql(
  ...
  file { "${base}/etherpad-lite/create_database.sh":
    ensure  => present,
  ...
  file { "${base}/etherpad-lite/create_user.sh":
    ensure  => present,
  ...
  exec { 'create-etherpad-lite-db':
    ...
    command => "${base}/etherpad-lite/create_database.sh",
    require => [
      Service['mysql'],
  ...
  exec { 'grant-etherpad-lite-db':
    ...
    command => "${base}/etherpad-lite/create_user.sh",
    require => [
      Service['mysql'],
      File["${base}/etherpad-lite/settings.json"],
      File["${base}/etherpad-lite/create_user.sh"],
