...
#
# Actions:
# - Prepare apache vhost and create mysql database
...
class drupal (
  ...
  $drupal_related_packages = [ "unzip", "php5-mysql", "php5-gd", "php5-cli",
    'libapache2-mod-php5' ]
  package { $drupal_related_packages: ensure => "installed",
    require => Package['httpd'],
  ...
  distbuild { "distbuild-${site_name}":
    ...
    site_build_repo_name => $site_build_repo_name,
    site_staging_tarball => $site_staging_tarball,
    site_build_flagfile  => $site_build_flagfile,
    require              => [ Package['httpd'], Mysql::Db["${site_mysql_database}"] ],
  }
