...
define drupal::distbuild (
  ...
  vcsrepo { "${site_sandbox_root}/${$site_build_reponame}":
    ensure   => latest,
    provider => git,
  ...
  exec { drupal-build-dist:
    path    => '/usr/bin:/bin',
    timeout => 900,
    cwd     => "${site_sandbox_root}/${$site_build_reponame}",
    command => "rm -rf ${site_staging_root}/${site_staging_tarball} && drush make --tar ${site_makefile} ${site_staging_root}/${site_staging_tarball}",
    unless  => "diff ${site_sandbox_root}/${$site_build_reponame}/.git/packed-refs ${site_build_flagfile}",
    require => [ Vcsrepo["${site_sandbox_root}/${$site_build_reponame}"], File["${site_staging_root}"] ]
  }

  exec { drupal-build-dist-post:
    path    => '/usr/bin:/bin',
    command => "cp ${site_sandbox_root}/${$site_build_reponame}/.git/packed-refs ${site_build_flagfile} && rm -rf ${site_deploy_flagfile}",
    unless  => "diff ${site_sandbox_root}/${$site_build_reponame}/.git/packed-refs ${site_build_flagfile}",
