...
logging.basicConfig(
            ...
            filename='/var/log/gerrit/update_users.log')
    ...
    def get_all_users(self):
        ...
        for team in self.lp_teams:
            for detail in self.launchpad.people[team].members_details:
                if (detail.status == 'Approved' or
                    detail.status == 'Administrator'):
                    name = detail.self_link.split('/')[-1]
                    if ((users.count(name) == 0) and
                        (name not in self.lp_teams)):
                        users.append(name)
        return users
                ...
                if name in self.lp_teams:
                    ...
                    for implied_group in gerrit.get_implied_groups(name):
                        if implied_group in self.lp_teams:
                            users.extend(self.get_team_members(implied_group,
                                                               gerrit))
...
class GerritAction(object):
    def __init__(self):
        ...
        self.ssh.connect('localhost', username=GERRIT_USER, port=29418,
    ...
    #   trying to get stdout return code or stderr can hang with large
    #   result sets
    #    for line in stderr:
    #        logger.error(line)
        return stdout
    ...
    def get_group_id(self, group_name):
        query = "select group_id from account_groups \
                 where name='{0}'".format(group_name)
        stdout = self.run_query(query)
    ...
    def get_user_id(self, user_name):
        query = "select account_id from account_external_ids where \
                 external_id='username:{0}'".format(user_name)
        stdout = self.run_query(query)
        line = stdout.readline()
        row = json.loads(line)
    ...
    def get_users_from_group(self, group_name):
        ...
        query = "select external_id from account_external_ids \
                 join account_group_members on \
                 account_group_members.account_id=\
                 account_external_ids.account_id \
                 where account_group_members.group_id={0} \
                 and external_id like 'username%%'".format(gid)
        stdout = self.run_query(query)
        for line in stdout:
            row = json.loads(line)
            if row['type'] == 'row':
    ...
    def get_users_from_watches(self, group_name):
        ...
        query = "select external_id from account_external_ids \
                 join account_project_watches on \
                 account_project_watches.account_id=\
                 account_external_ids.account_id \
                 where account_project_watches.project_name like '{0}' \
                 and external_id like 'username%%'".format(group_name)
        stdout = self.run_query(query)
        for line in stdout:
            row = json.loads(line)
            if row['type'] == 'row':
    ...
    def get_implied_groups(self, group_name):
        ...
        groups = []
        query = "select name from account_groups \
                 join account_group_includes on \
                 account_group_includes.include_id=account_groups.group_id \
                 where account_group_includes.group_id={0}".format(gid)
        stdout = self.run_query(query)
        for line in stdout:
            row = json.loads(line)
            if row['type'] == 'row':
    ...
    def add_group(self, group_name, group_display_name):
        ...
        query = "insert into account_groups \
                 (group_id, group_type, owner_group_id, name, \
                  description, group_uuid) \
                 values ({0}, 'INTERNAL', 1, '{1}', '{2}', '{3}')"\
                 .format(gid, group_name, group_display_name, full_uuid)
        self.run_query(query)
        query = "insert into account_group_names (group_id, name) \
                 values ({0}, '{1}')".format(gid, group_name)
        self.run_query(query)
    ...
    def add_user(self, user_name, user_data):
        ...
        query = "select account_id from account_external_ids where \
                 external_id in ('{0}')".format(user_openid_external_id)
        stdout = self.run_query(query)
        row = json.loads(stdout.readline())
        if row['type'] == 'row':
            # we have a result so this is an updated user name
            account_id = row['columns']['account_id']
            query = "update account_external_ids set external_id='{0}' where \
                    external_id like 'username%%' and account_id = {1}"\
                    .format('username:%s' % user_name, account_id)
            self.run_query(query)
        else:
            # we really do have a new user
            user_ssh_keys = ["%s %s %s" %
                             ('ssh-%s' % key.keytype.lower(),
                              key.keytext,
                              key.comment) for key in user_data.sshkeys]
            user_email = None
            try:
            ...
            query = "insert into accounts \
                     (account_id, full_name, preferred_email) values \
                     ({0}, '{1}', '{2}')".format(uid, user_name, user_email)
            self.run_query(query)
            keyno = 1
            for key in user_ssh_keys:
                query = "insert into account_ssh_keys \
                         (ssh_public_key, valid, account_id, seq) values \
                         ('{0}', 'Y', {1}, {2})".format(key.strip(), uid,
                                                        keyno)
                self.run_query(query)
                keyno = keyno + 1
            query = "insert into account_external_ids \
                     (account_id, email_address, external_id) values \
                     ({0}, '{1}', '{2}')".format(uid, user_email,
                                                 user_openid_external_id)
            self.run_query(query)
            query = "insert into account_external_ids \
                     (account_id, external_id) values \
                     ({0}, '{1}')".format(uid, "username:%s" % user_name)
            self.run_query(query)
    ...
    def add_user_to_group(self, user_name, group_name):
        ...
        gid = self.get_group_id(group_name)
        if gid is 0:
            print "Trying to add user {0} \
                   to non-existent group {1}".format(user_name, group_name)
            raise
        query = "insert into account_group_members \
                 (account_id, group_id) values ({0}, {1})".format(uid, gid)
        self.run_query(query)

    def add_user_to_watch(self, user_name, group_name):
        logger.info("Adding Gerrit user %s to watch group %s", user_name,
                    group_name)
        uid = self.get_user_id(user_name)
        if group_name.endswith("-core"):
            group_name = group_name[:-5]
        group_name = "openstack/{0}".format(group_name)
        query = "insert into account_project_watches VALUES \
                 ('Y', 'N', 'N', {0}, '{1}', '*')". format(uid, group_name)
        self.run_query(query)

    def del_user_from_group(self, user_name, group_name):
        logger.info("Deleting Gerrit user %s from group %s", user_name,
                    group_name)
        uid = self.get_user_id(user_name)
        gid = self.get_group_id(group_name)
        query = "delete from account_group_members where account_id = {0} and \
                 group_id = {1}".format(uid, gid)
        self.run_query(query)
        if group_name.endswith("-core"):
            group_name = group_name[:-5]
        group_name = "openstack/{0}".format(group_name)
        query = "delete from account_project_watches where account_id = {0} \
                 and project_name= '{1}'".format(uid, group_name)
        self.run_query(query)
    ...
    def rebuild_sub_groups(self, group, sub_groups):
        ...
        for sub_group in sub_groups:
            ...
            query = "select group_id from account_group_includes where \
                     group_id={0} and include_id={1}".format(gid, sgid)
            stdout = self.run_query(query)
            row = json.loads(stdout.readline())
            if row['type'] != 'row':
                logger.info('Adding implied group %s to group %s', group,
                            sub_group)
                query = "insert into account_group_includes \
                         (group_id, include_id) values ({0}, {1})".format(gid,
                                                                          sgid)
                self.run_query(query)
...
for group in lp_groups:
    ...
    group_diff = filter(lambda a:
                        a not in gerrit_group_watches, lp_group_watches)
    for user in group_diff:
