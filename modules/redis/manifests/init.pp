...
$redis_bind='127.0.0.1') {
  ...
  case $redis_version {
     /2\.2\.\d+/: {
       $redis_conf_file='redis/redis.2.2.conf.erb'
    }
    /2\.4\.\d+/: {
       $redis_conf_file='redis/redis.2.4.conf.erb'
    }
    /2\.6\.\d+/: {
       $redis_conf_file='redis/redis.2.6.conf.erb'
    }
    default: {
  ...
  notify {
    "using redis ${redis_version} , config template ${redis_conf_file}":;}
