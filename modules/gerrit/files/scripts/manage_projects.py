...
def create_groups_file(project, host, port, repo_path):
    ...
    for line in open(acl_config, 'r'):
        ...
        if r:
            group = r.group(1)
            cmd = "python /usr/local/gerrit/scripts/get_group_uuid.py " \
                  "--host %s --port %s %s" % (host, port, group)
            ret, out = run_command_status(cmd)
            if ret:
                uuids[group] = out
            else:
                return False
    if uuids:
        ...
        for group, uuid in items(uuids):
            fp.write("%s\t%s"% (uuid, group))
        fp.close()
    return True
    ...
    if 'acl_config' in section:
        ...
        try:
            ...
            if (fetch_config(project, remote_url, repo_path) and
                ...
                create_groups_file(project, "localhost", 29418, repo_path)):
