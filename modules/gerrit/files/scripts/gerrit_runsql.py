...
# TODO: implement a more resiliant find java,
# maybe a find binary that itterates the Path
def find_java():
    return which('java')
...
def main():
    ...
    parser = argparse.ArgumentParser(description='run gerrit ' +
             ' gsql commands from yaml.')
    parser.add_argument('--loglevel',
     help='Specify the default logging level (optional).',
     choices=['debug', 'info', 'warning',
              'error', 'DEBUG', 'INFO', 'WARNING', 'ERROR'], default='info')
    parser.add_argument('--logfile',
     help='Specify logfile name.', default='/tmp/gerrit_runsql.log')
    parser.add_argument('--loggername',
     help='Specify a name for the logger.', default=LOGGER_NAME)
    ...
    parser.add_argument('--onlyif_not_hasrows',
     help='only run if the sql does not have rows.', default='')
    parser.add_argument('--check',
     help='only echo sql we will execute, do not run it',
     action='store_true', default=False)
    parser.add_argument('--sql_config_file',
