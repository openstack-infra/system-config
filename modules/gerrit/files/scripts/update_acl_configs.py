...
def update_repo(project, repo_root):
    git_dir = os.path.join(repo_root, project, '.git')

    status, _ = run_command("git --git-dir=%s fetch origin +refs/meta/config:"
                            "refs/remotes/gerrit-meta/config" %
                            git_dir, status=True)
    if status != 0:
        print "Failed to fetch refs/meta/config for project: %s" % project
        return False
    status, _ = run_command("git --git-dir=%s checkout -b config "
...
def copy_acl_config(project, repo_root, acl_config):
    ...
    if status == 0:
       status, _ = run_command("git --git-dir=%s diff-index --quiet HEAD --" %
                                git_dir, status=True)
       if status != 0:
           return True
    return False
...
def push_acl_config(project, repo_root):
    ...
    status, _ = run_command("git --git-dir=%s commit -a -m"
                            "'Update project config.'" %
                            git_dir, status=True)
    if status != 0:
        print "Failed to commit config for project: %s" % project
        return False
    status, _ = run_command("git --git-dir=%s push origin "
...
def checkout_master(project, repo_root):
    git_dir = os.path.join(repo_root, project, '.git')
    run_command("git --git-dir=%s checkout master", git_dir)
    run_command("git --git-dir=%s branch -D config", git_dir)
