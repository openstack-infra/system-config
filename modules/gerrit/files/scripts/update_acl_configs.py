...
def update_repo(project, repo_root):
    ...
    status, _ = run_command("git checkout --git-dir=%s meta/config" %
                            project_dir, status=True)
    if status != 0:
        return False
    status, _ = run_command("git pull --ff-only --git-dir=%s "
                            "origin meta/config" % project_dir, status=True)
    if status != 0:
        return False
...
def push_acl_config(project, repo_root):
    project_dir = os.path.join(repo_root, project)
    status, _ = run_command("git push --git-dir=%s origin meta/config" %
                            project_dir, status=True)
    if status == 0:
        return True
    return False
...
for section in config:
    ...
    if (update_repo(project, repo_root) and
        ...
        if push_acl_config(project, repo_root):
            exit_status = 1
    else:
        exit_status = 1
