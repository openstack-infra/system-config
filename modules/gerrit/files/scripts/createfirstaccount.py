...
# under the License.
# create the first account, we need this to automate
# setup of our batch accounts
...
# TODO: implement a more resiliant find java, maybe
# a find binary that itterates the Path
def find_java():
    return which('java')


def find_gerritwar():
    return os.path.join(get_ux_home('gerrit2'),
...
def gsql_exec(sqlcmd):
    ...
    try:
        try:
            ...
            # example
            # /usr/bin/java -jar
            #       /home/gerrit2/review_site/bin/gerrit.war
            #      gsql -d /home/gerrit2/review_site -c 'show tables'
            results = exec_cmd(java_bin, ["-jar",
                                          gerrit_war,
                                          "gsql",
                                          "-d",
                                          gerrit_site,
...
def main():
    ...
    parser.add_argument('--username',
                        help='change default user name to create as' +
                             ' first account.',
                        default='gerrit2')
    parser.add_argument('--email',
                        help='specify the email address for the user.',
                        default='gerrit2@localhost.com')
    parser.add_argument('--ssh_pubkey',
                        help='pupblic key to use. Example, generate with :\n' +
                        'ssh-keygen -t rsa  -f ~/.ssh/gerrit2 -P ""',
                        default='/home/gerrit2/.ssh/gerrit2.pub')
    parser.add_argument('--check_exists',
                        help='just check if the first account exist,' +
                             ' if it does not,' +
    ...
    if args.check_exists:
        ...
        if retval[1].upper().rstrip('\n').find('0 ROWS') >= 0:
            logger.info("check found that the account does not exist," +
                        " return 0.")
            banner_end()
            return 0
        else:
    ...
    if retval[1].upper().rstrip('\n').find('0 ROWS') >= 0:
        sql_commands = [
            ...
            " (REGISTERED_ON, FULL_NAME) VALUES (NOW(), '" +
            args.username + "')",
            "INSERT INTO account_group_members" +
            " (ACCOUNT_ID, GROUP_ID) VALUES (0, 1)",
            "INSERT INTO account_external_ids" +
            " (ACCOUNT_ID, EXTERNAL_ID) VALUES" +
            " (0, 'gerrit:" + args.username + "')",
            "INSERT INTO account_external_ids" +
            " (ACCOUNT_ID, EXTERNAL_ID) VALUES" +
            " (0, 'username:" + args.username + "')",
            "INSERT INTO account_ssh_keys" +
            " (SSH_PUBLIC_KEY, VALID) VALUES ('" +
            ssh_pub_key_content + "', 'Y')",
            "update account_external_ids" +
            " set email_address='" +
            args.email + "' where external_id like '%:" +
            args.username + "'"]
