...
def exec_cmd(binary, arguments='', current_pwd='/tmp', ignore_err=False):
    ...
    try:
        ...
        p = subprocess.Popen([binary] + arguments,
                             stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        out, err = p.communicate()
        if not ignore_err:
            if len(err) > 0:
                logger.error("Command " + ' '.join([binary] +
                             arguments) + " failed!! ")
                logger.error(err)
            else:
                logger.debug("Command " + ' '.join([binary] +
                             arguments) + " passed ")
                logger.debug(out)
        else:
...
def exec_return_exit(binary, arguments='', current_pwd='/tmp'):
    ...
    try:
        ...
        if not isinstance(arguments, list):
            arguments = arguments.split(' ')
        logger.debug("calling exec_cmd : " +
                     ' '.join([binary] + arguments))
        retcode = subprocess.call(' '.join([binary] + arguments),
                                  shell=True, cwd=current_pwd)
        return retcode
    except OSError as e:
        logger.error("Execution failed: " + str(e))
        return -1
