...
#   replication_targets:
#     A list of hashes describing additional replication stanzas. Each must
#     have a name key, and then the remaining key,value pairs will be added
...
# to be managed in here when we get project creation handled
    file { '/home/gerrit2/review_site/etc/replication.config':
      ...
      content => template('gerrit/replication.config'),
      replace => 'true',
