...
class gerrit(
  ...
  exec { 'gerrit-initial-init':
    user      => 'gerrit2',
    command   => "/usr/bin/java -jar ${gerrit_war} init -d ${gerrit_site} --batch --no-auto-start; test x${secondary_index} = xtrue && /usr/bin/java -jar ${gerrit_war} reindex -d ${gerrit_site}",
  ...
  exec { 'gerrit-init':
    user        => 'gerrit2',
    command     => "/etc/init.d/gerrit stop; /usr/bin/java -jar ${gerrit_war} init -d ${gerrit_site} --batch --no-auto-start; test x${secondary_index} = xtrue && /usr/bin/java -jar ${gerrit_war} reindex -d ${gerrit_site}",
    subscribe   => File['/home/gerrit2/review_site/bin/gerrit.war'],
    refreshonly => true,
    require     => [Package['openjdk-7-jre-headless'],
                    User['gerrit2'],
