...
class gerrit(
  ...
  $ssh_replication_key_contents = '', # If left emptry will not create files.
  $ssh_replication_pubkey_contents = '', # If left emptry will not create files.
  ...
  file { '/home/gerrit2/review_site/.ssh':
    ensure  => directory,
    owner   => 'gerrit2',
    mode    => '0700',
  ...
  if $ssh_replication_key_contents != '' {
    file { '/home/gerrit2/.ssh/id_rsa':
      owner   => 'gerrit2',
      group   => 'gerrit2',
      mode    => '0600',
      content => $ssh_replication_key_contents,
      replace => true,
      require => File['/home/gerrit2/.ssh']
    }
  }

  if $ssh_replication_pubkey_contents != '' {
    file { '/home/gerrit2/id_rsa.pub':
      owner   => 'gerrit2',
      group   => 'gerrit2',
      mode    => '0644',
      content => $ssh_replication_pubkey_contents,
      replace => true,
      require => File['/home/gerrit2/.ssh']
    }
  }
