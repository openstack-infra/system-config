class pypimirror ( $base_url,
                   ...
                   $mirror_file_path = "/usr/local/pypimirror",
                   $fetch_since_days = 1,
                   $package_matches = ["*"],
                   $external_links = true,
                   $follow_external_index_pages = false )
{
  ...
  exec { "update_z3c.pypimirror":
    command => "hg pull",
    cwd => "/tmp/z3c.pypimirror",
    path => "/bin:/usr/bin",
    onlyif => "test -d /tmp/z3c.pypimirror",
  ...
  exec { "get_z3c.pypimirror":
    command => "hg clone https://bitbucket.org/rsyring/z3c.pypimirror /tmp/z3c.pypimirror",
    path => "/bin:/usr/bin",
    onlyif => "test ! -d /tmp/z3c.pypimirror"
  }
