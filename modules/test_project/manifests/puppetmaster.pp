# == Class: test_project::puppetmaster
#
class test_project::puppetmaster (
  $jenkins_api_key = '',
  ...
  $root_rsa_key = 'xxx',
  $puppetdb = true,
  $puppetdb_server = 'puppetdb.test.com',
) {
  include logrotate
  include test_project::params
  include test_project::server
  ...
  file {'/etc/puppet/environments/production/environment.conf':
    ...
    group  => 'root',
    mode   => '0644',
    source => 'puppet:///modules/test_project/puppetmaster/production_environment.conf',
  }
...
#  }
  ...
  file { '/etc/puppet/hiera.yaml':
    ...
    group   => 'root',
    mode    => '0555',
    source  => 'puppet:///modules/test_project/puppetmaster/hiera.yaml',
    replace => true,
...
# For puppet master apache serving.
  ...
  file { '/etc/apache2/sites-available/puppetmaster.conf':
    ...
    group   => 'root',
    mode    => '0600',
    content => template('test_project/puppetmaster/puppetmaster_vhost.conf.erb'),
...
# with non-ascii chars
  file { '/etc/apache2/envvars':
    ...
    group   => 'root',
    mode    => '0444',
    source  => 'puppet:///modules/test_project/puppetmaster/envvars.debian',
    require => Package['puppetmaster-passenger'],
  }
...
#
  file { '/etc/ansible/playbooks':
    ...
    recurse => true,
    source  => 'puppet:///modules/test_project/ansible/playbooks',
    require => Class[ansible],
  }
