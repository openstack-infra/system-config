# == Class: test_project::ask_dev
#
# ask-staging.openstack.org Q&A support website
#
class test_project::ask_staging (
  $db_password,
  $redis_password,
  $db_name          = 'askbotdb',
  $db_user          = 'ask',
  ...
  file { '/usr/share/solr/core-en/conf/schema.xml':
    ...
    content => template('test_project/askbot/schema.en.xml.erb'),
    replace => true,
    owner   => 'jetty',
    group   => 'jetty',
    mode    => '0644',
  ...
  file { '/usr/share/solr/core-zh/conf/schema.xml':
    ...
    content => template('test_project/askbot/schema.cn.xml.erb'),
    replace => true,
    owner   => 'jetty',
    group   => 'jetty',
    mode    => '0644',
  ...
  class { 'askbot':
    ...
    template_settings     => 'test_project/askbot/settings.py-staging.erb',
  }
