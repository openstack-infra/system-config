...
class test_project::template (
  ...
  $enable_unbound            = true,
  $afs                       = false,
  $puppetmaster_server       = 'puppetmaster.test.com',
  $manage_exim               = false,
  $sysadmins                 = [],
  ...
  class {'test_project::users_install':
  ...
  if $::osfamily == 'Debian' {
    ...
    file { '/etc/rsyslog.d/50-default.conf':
      ...
      mode    => '0644',
      source  =>
        'puppet:///modules/test_project/rsyslog.d_50-default.conf',
      replace => true,
      notify  => $rsyslog_notify,
      require => Package['rsyslog'],
    }
  ...
  package { $::test_project::params::packages:
    ensure => present
  }
  ...
  ###########################################################
  # Manage Puppet
  # possible TODO: break this into test_project::puppet
  ...
  if ($::osfamily == 'Debian') {
    ...
    file { '/etc/apt/apt.conf.d/80retry':
      ...
      group   => 'root',
      mode    => '0444',
      source  => 'puppet:///modules/test_project/80retry',
      replace => true,
    }
    ...
    file { '/etc/apt/apt.conf.d/90no-translations':
      ...
      group   => 'root',
      mode    => '0444',
      source  => 'puppet:///modules/test_project/90no-translations',
      replace => true,
    }
    ...
    file { '/etc/apt/preferences.d/00-puppet.pref':
      ...
      group   => 'root',
      mode    => '0444',
      content => template('test_project/00-puppet.pref.erb'),
      replace => true,
    }
    ...
    file { '/etc/default/puppet':
      ...
      group   => 'root',
      mode    => '0444',
      source  => 'puppet:///modules/test_project/puppet.default',
      replace => true,
    }
  ...
  if ($::operatingsystem == 'CentOS') {
    if ($::operatingsystemmajrelease == '6') {
      $puppet_repo_source_path =
        'puppet:///modules/test_project/centos6-puppetlabs.repo'
      $custom_cgit = present
    } elsif ($::operatingsystemmajrelease == '7') {
      $puppet_repo_source_path =
        'puppet:///modules/test_project/centos7-puppetlabs.repo'
      $custom_cgit = absent
    }
    ...
    file { '/etc/yum.repos.d/git-1237395.repo':
      ...
      group   => 'root',
      mode    => '0444',
      source  => 'puppet:///modules/test_project/git-1237395.repo',
      replace => true,
    }
    ...
    file { '/etc/yum.conf':
      ...
      group   => 'root',
      mode    => '0444',
      source  => 'puppet:///modules/test_project/yum.conf',
      replace => true,
    }
  }
  ...
  file { '/etc/puppet/puppet.conf':
    ...
    group   => 'root',
    mode    => '0444',
    content => template('test_project/puppet.conf.erb'),
    replace => true,
  }
