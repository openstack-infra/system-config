...
#
# Common configuration between test_project::slave and
# test_project::single_use_slave
class test_project::slave_common(
  $sudo         = false,
  $project_config_repo = '',
){
  ...
  file { '/home/jenkins/.pydistutils.cfg':
    ...
    group   => 'jenkins',
    mode    => '0644',
    source  => 'puppet:///modules/test_project/pydistutils.cfg',
  ...
  if ($sudo == true) {
    file { '/etc/sudoers.d/jenkins-sudo':
      ...
      source => 'puppet:///modules/test_project/jenkins-sudo.sudo',
      owner  => 'root',
      group  => 'root',
      mode   => '0440',
    }
  ...
  file { '/etc/sudoers.d/jenkins-sudo-grep':
    ...
    source => 'puppet:///modules/test_project/jenkins-sudo-grep.sudo',
    owner  => 'root',
    group  => 'root',
    mode   => '0440',
  }
  ...
  file { '/etc/zuul-env-reqs.txt':
    ...
    group  => 'root',
    mode   => '0444',
    source => 'puppet:///modules/test_project/zuul-env-reqs.txt',
  }
