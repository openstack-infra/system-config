...
define askbot::site (
  ...
  file { "${askbot_site_root}/config/__init__.py":
    ensure  => present,
    owner   => 'root',
    group   => 'root',
    mode    => '0644',
  ...
  file { "${askbot_site_root}/config/manage.py":
    ensure  => present,
    owner   => 'root',
    group   => 'root',
    mode    => '0644',
  ...
  file { "${askbot_site_root}/config/urls.py":
    ensure  => present,
    owner   => 'root',
    group   => 'root',
    mode    => '0644',
  ...
  # Notice: do we need this here?
  file { "${askbot_site_root}/config/django.wsgi":
    ensure  => present,
    owner   => 'root',
    group   => 'root',
    mode    => '0644',
    source  => 'puppet:///modules/askbot/setup_templates/django.wsgi',
    require => File["${askbot_site_root}/config"],
  }
