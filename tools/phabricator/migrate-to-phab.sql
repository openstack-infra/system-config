...
update task_count set story_id = story_id + 3000 where story_id < 3000;
...
update task_count set story_id = story_id - 2000000 + 4000 where story_id >= 2000000;
...
insert into maniphest_transaction
  select
    c.id as id,
    c.transPHID as phid,
    u.phid as authorPHID,
    s.phid as objectPHID,
    'public' as viewPolicy,
    u.phid as editPolicy,
    c.phid as commentPHID,
    1 as commentVersion,
    'core:comment' as transactionType,
    'null' as oldValue,
    'null'as newValue,
    '{"source":"web"}' as contentSource,
    '[]' as metadata,
    unix_timestamp(c.created_at) as dateCreated,
    if(c.updated_at is null, unix_timestamp(c.created_at), unix_timestamp(c.updated_at)) as dateModified
  ...
  where c.id = e.comment_id and s.id = e.story_id
     and e.event_type='user_comment' and e.author_id = u.id;
...
insert into maniphest_transaction_comment
  select
    c.id as id,
    c.phid as phid,
    c.transPHID as transactionPHID,
    u.phid as authorPHID,
    'public' as viewPolicy,
    u.phid as editPolicy,
    1 as commentVersion,
    if(c.content is NULL, '', c.content) as content,
    '{"source":"web"}' as contentSource,
    if(c.is_active, false, true) as isDeleted,
    unix_timestamp(c.created_at) as dateCreated,
    if(c.updated_at is null, unix_timestamp(c.created_at), unix_timestamp(c.updated_at)) as dateModified
  from storyboard.comments c, storyboard.events e, storyboard.stories s, storyboard.users u
  where c.id = e.comment_id and s.id = e.story_id
