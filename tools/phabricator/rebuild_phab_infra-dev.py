...
def rebuildstoryboard():
    ...
    if 'storyboard' in dbs:
        print('storyboard database exists, dropping and creating')
        dropcreatestoryboard()
    else:
        print('storyboard database does not exist, creating')
        sql = "CREATE DATABASE storyboard"
        CURSOR.execute(sql)
    print('Importing the Storyboard database from backup...')
...
def dbmigration():
    ...
    print('Migrating Storyboard to Phabricator...')
    #migrate = open('/home/ubuntu/puppet-phabricator/migrate-to-phab.sql', 'r')
    #CURSOR.execute(file.read(migrate))
    #migrate = "mysql -u root < /home/ubuntu/puppet-phabricator/migrate-to-phab.sql"
    #subprocess.call([migrate])
    #subprocess.call(["/usr/bin/mysql", "-u", "root", "<", "/home/ubuntu/puppet-phabricator/migrate-to-phab.sql"])
    os.system('/usr/bin/mysql -u root < /home/ubuntu/puppet-phabricator/migrate-to-phab.sql')
