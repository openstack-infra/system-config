...
def admincraige():
    ...
    # Link Craige to his OpenID account until it's sorted for everyone
    sql = ('UPDATE phabricator_user.user SET username = \'craige\' ' +
           'WHERE phid = \'%s\'' % phid_user)
    print(sql)
    CURSOR.execute(sql)
    MySQL.commit()
    sql = (('INSERT INTO phabricator_user.user_externalaccount ( ' +
           'phid, userPHID, accountType, accountDomain, accountID) ' +
           'SELECT \'PHID-XUSR-dgkm64utdjqfvsqzi6gn\' as phid, \'%s\' ' +
           'as userPHID, \'RemoteUser\' as accountType, \'self\' as ' +
           'accountDomain, \'https://login.ubuntu.com/+id/7yHwYcL\' as ' +
           'accountID') % phid_user)
    print(sql)
    CURSOR.execute(sql)
    MySQL.commit()
    ...
    os.system('%s %s/bin/auth recover craige' % (WEBDO, PHABROOT))
