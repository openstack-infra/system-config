...
class PuppetUtilMetric(yaml.YAMLObject):
...
class PuppetTransactionReport(yaml.YAMLObject):
...
class PuppetUtilLog(yaml.YAMLObject):
...
def main():
    ...
    results = yaml.load(open(args.logfile, 'r').read())
    if results.failed():
