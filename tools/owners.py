...
#     >>> import yaml
#     >>>
#     >>> o = yaml.load(open('_all_owners.yaml'))
#     >>> for c in range(5):
...
# TODO(fungi): delete this once everything's working correctly
def debug(message):
    """ Convenience wrapper around print() to make debugging cruft
    easier to find and remove later"""
...
def date_merged(change, after=None, before=None):
    ...
    for message in reversed(change['messages']):
        if (('author' not in message) or (
                message['author']['_account_id'] == 3 and message['message'] ==
                'Change has been successfully merged into the git '
                'repository.')):
            date = message['date']
            break
...
def main():
    ...
    parser.add_argument("-i", "--ignore", help="Account Ids to skip",
                        action='append')
    ...
    # Path to the governance projects list, needs a Git refname as a
    # parameter
    EXTRA_ATCS_LIST = ('https://review.openstack.org/'
                       'gitweb?p=openstack/governance.git;a=blob_plain;'
                       'f=reference/extra-atcs;hb=%s')
    ...
    # The set of projects from the governance repo
    # TODO(fungi): make this a configurable option so that you can
    # for example supply a custom extra-atcs list
    gov_extra_atcs = requests.get(
        EXTRA_ATCS_LIST % ref).text.encode('utf-8').split('\n')
    ...
    # This will be populated with a mapping from normalized
    # project-team names to their originals from governance, for use
    # in lookups from the extra-atcs list which can be inconsistent
    # in places
    normalized_projects = {}
    ...
    for project in gov_projects:
        ...
        # Track the normalized name as well
        normalized_projects[normalize_project(project)] = project
    ...
    # The negative linenumber in the extra-atcs file will be used as
    # a makeshift account Id for non-code contributors; those with
    # owned changes use their Gerrit account Id instead
    lineno = 1
    ...
    # TODO(fungi): use https://review.openstack.org/267143 instead
    for line in gov_extra_atcs:
        match = re.match('([^#][^:]*): (.+) \((.+@.+)\)', line)
        if match:
            team, name, email = match.groups()
            ...
            project = normalized_projects.get(normalize_project(team), team)
            if project not in projects:
                projects[project] = {}
                normalized_projects[normalize_project(project)] = project
                print('Adding project %s only in extra-atcs list' % project,
                      file=sys.stderr)
