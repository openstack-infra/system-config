...
CONFIG_FILE = '../modules/openstack_project/templates/nodepool/nodepool.yaml.erb'
    ...
    with open(config, 'r') as stream:
        ...
        for label in config['labels']:
            for provider in label['providers']:
                ret.append((label['name'], provider['name']))
    logging.debug("Added %d images & providers" % len(ret))
    return ret


def generate_logger_and_handler(image, provider):
    handler = IMAGE_HANDLER % {
        'title' : '%s_%s' % (image, provider),
        'filename' : '%s.%s.log' % (image, provider),
    }
    logger = IMAGE_LOGGER % {
        'title' : '%s_%s' % (image, provider),
        'handler' : '%s_%s' % (image, provider),
        'qualname' : '%s.%s' % (image, provider),
    }

    return {
        'handler_title' : '%s_%s' % (image, provider),
        'logger_title' : '%s_%s' % (image, provider),
        'handler' : handler,
        'logger' : logger,
    }
...
def main():
    ...
    for (image, provider) in get_images_and_providers(args.config):
        loggers_and_handlers.append(
            generate_logger_and_handler(image, provider))
