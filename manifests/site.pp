...
node 'wiki-dev.openstack.org' {
  class { 'openstack_project::wiki':
    mysql_root_password        => hiera('wiki_dev_db_password'),
    sysadmins                  => hiera('sysadmins', []),
    ssl_cert_file_contents     => hiera('wiki_dev_ssl_cert_file_contents'),
    ssl_key_file_contents      => hiera('wiki_dev_ssl_key_file_contents'),
    ssl_chain_file_contents    => hiera('wiki_dev_ssl_chain_file_contents'),
    wg_dbpassword              => hiera('wiki_dev_wg_dbpassword'),
    wg_secretkey               => hiera('wiki_dev_wg_secretkey'),
    wg_upgradekey              => hiera('wiki_dev_wg_upgradekey'),
    wg_captchaquestions        => hiera('wiki_dev_wg_captchaquestions'),
    wg_googleanalyticsaccount  => hiera('wiki_dev_wg_googleanalyticsaccount'),
  }
}
