...
for MOD in ${!SOURCE_MODULES[*]} ; do
    ...
    if ! echo $MODULE_LIST | grep "${MODULE_NAME}" >/dev/null 2>&1; then
        ...
        if [ ! -d ${MODULE_PATH}/${MODULE_NAME}/.git ]; then
            ...
            git clone $MOD "${MODULE_PATH}/${MODULE_NAME}"
        elif [ `${GIT_CMD_BASE} remote show origin | \
                grep 'Fetch URL' | \
                awk -F'URL: ' '{print $2}'` != $MOD ]; then
    ...
    # make sure the correct revision is installed, I have to use rev-list
    # b/c rev-parse does not work with tags
    if [ `${GIT_CMD_BASE} rev-list HEAD --max-count=1` != \
         `${GIT_CMD_BASE} rev-list ${SOURCE_MODULES[$MOD]} --max-count=1` ]; then
